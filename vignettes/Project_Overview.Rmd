...
levelplot(weekly_prec, 
          scales=list(draw=FALSE), #get rid of axes
          par.settings = viridisTheme(axis.line = list(col = "transparent")), #get rid of boundary
          main = "Weekly Mean Percipitation (10/27/2019 - 04/13/20)", 
...
server <- function(input, output, session) {
  ...
  df_filtered <- reactive({ #object for updating data display
    ...
    dec_class[[input$slider]] #filter data based on slider input and only keep non-NA pixels
...
Deciles: Deciles does not seem like a great way to analyze rainfall across space. I think these maps confirm that the eastern half of South Africa receives more rainfall than the western half. Having looked at some maps of ecoregions in South Africa, it seems that the eastern portion of South Africa has a drier climate and more arid landscape overall. Since we are broadly interested in A potential improvement for when using this index is to limit the geogrpahic extent to arabale land or to regions with large amounts of agriculture. 
